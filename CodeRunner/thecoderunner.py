...
class Obstacle(pygame.sprite.Sprite):
    def __init__(self,type):
        ...
        if type == 'fly':
            ...
            dragon = pygame.image.load('graphics/dragon/birdsprite.png')
            dragon = pygame.transform.scale(dragon, (100, 50))
            dragon = dragon.convert_alpha()
            self.frames = [dragon, dragon]
            self.speed = 20
            y_pos = 200
...
def stop_current_level(obstacle_group, player):
    for obstacle in obstacle_group: obstacle.kill()
    player.sprite.reset_attributes()
    return False
    ...
    for event in events:
        ...
        if game_active:                       
            if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE:
                game_active = stop_current_level(obstacle_group, player)
                if level < len(levels.texts) - 1 and score >= levels.reqs[level]: 
                    text_counter, level = next_level(level)
        ...
        if game_active:
            if event.type == obstacle_timer:
                obstacle_group.add(Obstacle(random.choices(levels.enemies[level], levels.probabilities[level])[0]))
